	...
	,	updateMultiShipToStatus: function (e)
		{
			...
			var non_shippable_lines = this.wizard.model.getNonShippableLines()
			,	application = this.wizard.application;

			// THIS CHECK IS MADE JUST AS A WORKAROUND FOR A WP BUG
			if (non_shippable_lines.length)
			{
				this.packageDetailsView = new NonShippalbeItemsView({
					model: this.wizard.model
				,	application: application
				,	parent: this
				});

				application.getLayout().showInModal(this.packageDetailsView);
			}
			else if(this.wizard.model.get('promocode'))
			{
