	...
	,	search: function (keywords)
		{
			...
			if (this.getApplication().getConfig('isSearchGlobal') || !(currentView && currentView.options.translator instanceof Translator))
			{
				...
				{
					var rawFragment = Backbone.history.fragment.split("&keywords=")[0]; // removes page from params when searching
					if (rawFragment.indexOf('page=') > 0){
						var index = rawFragment.indexOf('page=');
						rawFragment = rawFragment.replace(rawFragment.substr(index).split("?")[0], "");
					}
