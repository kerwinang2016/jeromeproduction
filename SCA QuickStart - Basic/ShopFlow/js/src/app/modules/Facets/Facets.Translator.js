...
,	function ()
{
	...
	// Categories is not a real dependency, so if it is present we use it other ways we dont
	...
	,	sanitizeValue: function (value, behavior)
		{
			...
			behavior = (!_.isNullOrEmpty(behavior)) ? behavior : 'single';
			...
			default:
				//parsed_value = value;
				parsed_value = (!_.isNullOrEmpty(value)) ? value : '';
	...
	,	parseUrlFacet: function (name, value)
		{
			...
			this.facets.push({
				config: config
			//,	id: config.id
			,	id: (config.id == 'category') ? 'custitem_clothing_type' : config.id
			,	url: config.url
			,	value: this.sanitizeValue(value, config.behavior)
			,	priority: config.priority
			}
			//, [{priority:config.priority, behavior:config.behavior, value:this.sanitizeValue(value, config.behavior)}]
			);
		}
	...
	,	cloneWithoutFacetId: function (facet_id)
		{
			...
			var isCategory = (facet_id == 'category') ? true : false;
			...
			if (isCategory)
			{
				//facets = [{config: {id: 'category', name: 'Category', behavior: 'single'}, id: 'category', priority:10, behavior:'single', value:[]}]
				//facets = this.facets;
				facets = _.getFacetCategoryCloneWithoutFacetId(this.facets)
			}
	...
	,	cloneForOption: function (option_id, option_value)
		{
			...
			var isCategory = (this.facets[0]['config']['id'] == 'category') ? true : false;
			//var isCategory = true;

			var facets  = _.toArray(jQuery.extend(true, {}, this.facets));
			//var facets  = (isCategory) ? _.getFacetCategoryCloneForOptionOptions(this.facets) : _.toArray(jQuery.extend(true, {}, this.facets));
			var	options = jQuery.extend(true, {}, this.options);
			...
			**/
			...
			if (isCategory)
			{
				facets = _.getFacetCategoryCloneForOptionOptions(this.facets)
			}
