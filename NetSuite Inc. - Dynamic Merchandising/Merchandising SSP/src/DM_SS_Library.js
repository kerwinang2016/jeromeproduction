...
psg_dm.getAllSearchResults = function (record_type, filters, columns)
{
...
psg_dm.getMerchRule = function(zone) {
	...
	var all_rules_cache_key = 'all_rules6';

	if (!zone)
	{
		var all_rules_cache = psg_dm.getRuleFromCache(all_rules_cache_key, 'all_sites');

		logger.debug('RULES CACHE', all_rules_cache);
		
		if (all_rules_cache)
		{
			try
			{
				return JSON.parse(all_rules_cache);
			}
			catch(e){}
		}
	}
	...
	var result = psg_dm.getAllSearchResults('customrecord_merch_rule', filters, columns);	
...
//				merchRule[merchRuleName].include = psg_dm.getMerchIncludeItems(merchRuleId);
		...
		psg_dm.putRuleToCache(all_rules_cache_key, 'all_sites', JSON.stringify(merchRule));
	}
...
psg_dm.validateFieldset = function(fieldsetId, site) {
	...
	if (!psg_dm.validateFieldsetCache[site])
	{
		var filters = [ new nlobjSearchFilter('custrecord_site_name_fieldsets', null, 'is', site) ];
		var columns = [ new nlobjSearchColumn('custrecord_active_fieldsets'), new nlobjSearchColumn('custrecord_fieldset_id') ];		
		var result = psg_dm.getAllSearchResults('customrecord_merch_website_fieldsets', filters, columns);

		psg_dm.validateFieldsetCache[site] = {};
		...
		if (result) {
			...
			result.forEach(function(fieldsetRecord) {
				psg_dm.validateFieldsetCache[site][fieldsetRecord.getValue('custrecord_fieldset_id')] = fieldsetRecord.getValue('custrecord_active_fieldsets') == 'T';
			});		
		}
	}	
	
	// Check if fieldset is active. If active, sets fieldset name.
	if (psg_dm.validateFieldsetCache[site][fieldsetId]) {
		fieldset = fieldsetId;
	}
...
psg_dm.getMerchFilterCache = false;
...
psg_dm.getMerchFilter = function(merchRuleId, site) {
	...
	if (!psg_dm.getMerchFilterCache)
	{
		psg_dm.getMerchFilterCache = {}
		...
		var columns = [ new nlobjSearchColumn('custrecord_field_id_filter'),
		                ...
		                new nlobjSearchColumn('custrecord_merch_rule_name_filter'),
		                new nlobjSearchColumn('internalId').setSort() ];
		
		var result = psg_dm.getAllSearchResults('customrecord_merch_rule_filter', [], columns);
		...
		if (result)
		{
			...
			{
				if (!psg_dm.getMerchFilterCache[record.getValue('custrecord_merch_rule_name_filter')])
				{
					psg_dm.getMerchFilterCache[record.getValue('custrecord_merch_rule_name_filter')] = [];
				}

				psg_dm.getMerchFilterCache[record.getValue('custrecord_merch_rule_name_filter')].push(record)
	...
	var result = psg_dm.getMerchFilterCache[merchRuleId]
 ...
 */
psg_dm.getMerchSortCache = false;
psg_dm.getMerchSort = function(merchRuleId, site) {
	...
	if (!psg_dm.getMerchSortCache)
	{
		...
		var columns = [ new nlobjSearchColumn('custrecord_field_id_sort'),
		                ...
		                new nlobjSearchColumn('custrecord_merch_rule_name_sort'),
		                new nlobjSearchColumn('internalId').setSort() ];

		var result = psg_dm.getAllSearchResults('customrecord_merch_rule_sort', filters, columns);
		
		psg_dm.getMerchSortCache = {}

		if (result)
		{
			result.forEach(function(record)
			{
				if (!psg_dm.getMerchSortCache[record.getValue('custrecord_merch_rule_name_sort')])
				{
					psg_dm.getMerchSortCache[record.getValue('custrecord_merch_rule_name_sort')] = [];
				}

				psg_dm.getMerchSortCache[record.getValue('custrecord_merch_rule_name_sort')].push(record)
				
			});
		}
	}

	var result = psg_dm.getMerchSortCache[merchRuleId];
		...
		if(cache != null){
			try{
				if(key != null){
					...
					if (value.indexOf('[[MULTIPART]]:') === 0)
					{
						var part_keys = value.replace('[[MULTIPART]]:', '').split('|');
						value = '';
						part_keys.forEach(function(part_key)
						{
							value += cache.get(part_key);
						});
					}
		...
		if(cache != null){
			try{
				if(key != null && value != null){
					...
					var max_part_size = 100000;
					var putReturn;

					if (value.length > max_part_size)
					{
						var parts = Math.ceil(value.length / max_part_size)
						,	part_keys = [];

						for (var i = 0; i < parts; i++)
						{
							var part_value = value.substring(i * max_part_size, (i+1) * max_part_size)
							,	part_key = key + '_part_' + i;
 
							cache.put(part_key, part_value, getTTL());
							part_keys.push(part_key);
						}

						putReturn = cache.put(key, '[[MULTIPART]]:'+part_keys.join('|'), getTTL());						
						
					}
					else
					{
						putReturn = cache.put(key, value, getTTL());
					}
